...
function setSquare(event) {
  ...
  const neighbors = parseInt(document.querySelector("#chooseNeighbors").value);
  mineGrid[y][x].open = true;
  mineGrid[y][x].neighbors = neighbors;
    ...
    for (let j = 0; j < mineGrid[i].length; j++) {
      ...
      if (mineGrid[i][j].open == false) {
        if (flagMode.checked == true) {
          ...
          cell.addEventListener("contextmenu", setSquare);
        }
...
function revealNeighbors(mineGrid, i, j) {
  ...
  if (j > 0) {
    if (
      mineGrid[i][j - 1].neighbors == 0 &&
      mineGrid[i][j - 1].open == false &&
      mineGrid[i][j - 1].flag == false
    ) {
  ...
  if (i > 0 && j > 0) {
    if (
      mineGrid[i - 1][j - 1].neighbors == 0 &&
      mineGrid[i - 1][j - 1].open == false &&
      mineGrid[i - 1][j - 1].flag == false
    ) {
  ...
  if (i > 0) {
    if (
      mineGrid[i - 1][j].neighbors == 0 &&
      mineGrid[i - 1][j].open == false &&
      mineGrid[i - 1][j].flag == false
    ) {
...
function ruleOne(mineGrid) {
  ...
  for (let i = 0; i < mineGrid.length; i++) {
    for (let j = 0; j < mineGrid[i].length; j++) {
      if (
        mineGrid[i][j].edgeCount > 0 &&
        mineGrid[i][j].neighbors == mineGrid[i][j].edgeCount - probabilityZeroCount(mineGrid, i, j)
      ) {
        // Left
        if (j > 0) {
...
function ruleTwo(mineGrid) {
  ...
  for (let i = 0; i < mineGrid.length; i++) {
    for (let j = 0; j < mineGrid[i].length; j++) {
      if (
        mineGrid[i][j].edgeCount > 0 &&
        mineGrid[i][j].neighbors == probabilityHundredCount(mineGrid, i, j)
      ) {
        // Left
        if (j > 0) {
          if (mineGrid[i][j - 1].edge == true && mineGrid[i][j - 1].probability < 0) {
...
function canBeMine(mineGrid, grid, i, j) {
  ...
  if (j > 0) {
    if (
      mineGrid[i][j - 1].open == true &&
      mineGrid[i][j - 1].neighbors <=
        mineCount(grid, i, j - 1) + probabilityHundredCount(mineGrid, i, j - 1)
    ) {
      return false;
    }
  }
  ...
  if (i > 0 && j > 0) {
    if (
      mineGrid[i - 1][j - 1].open == true &&
      mineGrid[i - 1][j - 1].neighbors <=
        mineCount(grid, i - 1, j - 1) + probabilityHundredCount(mineGrid, i - 1, j - 1)
    ) {
      return false;
    }
  }
  ...
  if (i > 0) {
    if (
      mineGrid[i - 1][j].open == true &&
      mineGrid[i - 1][j].neighbors <=
        mineCount(grid, i - 1, j) + probabilityHundredCount(mineGrid, i - 1, j)
    ) {
      return false;
    }
  }
...
function canNotBeMine(mineGrid, grid, i, j) {
  ...
  if (j > 0) {
    if (
      mineGrid[i][j - 1].open == true &&
      mineGrid[i][j - 1].neighbors >=
        mineGrid[i][j - 1].edgeCount -
          noMineCount(grid, i, j - 1) -
          probabilityZeroCount(mineGrid, i, j - 1)
    ) {
      return false;
    }
  }
  ...
  if (i > 0 && j > 0) {
    if (
      mineGrid[i - 1][j - 1].open == true &&
      mineGrid[i - 1][j - 1].neighbors >=
        mineGrid[i - 1][j - 1].edgeCount -
          noMineCount(grid, i - 1, j - 1) -
          probabilityZeroCount(mineGrid, i - 1, j - 1)
    ) {
      return false;
    }
  }
  ...
  if (i > 0) {
    if (
      mineGrid[i - 1][j].open == true &&
      mineGrid[i - 1][j].neighbors >=
        mineGrid[i - 1][j].edgeCount -
          noMineCount(grid, i - 1, j) -
          probabilityZeroCount(mineGrid, i - 1, j)
    ) {
      return false;
    }
  }
...
function probabilityCalculation(edgeArr, mineGrid, allProbability) {
  ...
  for (i = 0; i < mineGrid.length; i++) {
    for (j = 0; j < mineGrid[i].length; j++) {
      ...
      if (
        mineGrid[i][j].open == false &&
        mineGrid[i][j].edge == false &&
        mineGrid[i][j].probability < 0
      ) {
